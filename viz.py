...
# %%
def run_poem_viz_doc(doc_name, text_dir, nll_dir, tokenizer, global_nll_min=None, global_nll_max=None):
    """
    ...
    for token, nll, norm_nll in zip(tokens, all_nlls, normalized_nlls):
        color = nll_to_rgb(norm_nll)
        display_token = token.replace('▁', ' ').replace('</s>', '').replace('<s>', '').replace('Ġ', '')
        if display_token.strip():
            combined_html += f'<span class="token" style="background-color: {color};" title="NLL: {nll:.4f}">{display_token}</span>'
...
def exp_poem():
    ...
    # run_poem_viz_doc('0', text_dir, nll_dir, tokenizer) 
